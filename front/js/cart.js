...
// Sinon on affiche les éléments du panier 
    ...
    function appearCart(api, cart){
        ...
        for(let products of cart){
            ...
            let apiData = api.find(element => element._id === products.id);
            ...
            cartItemContentSettingsQuantity.addEventListener("change",function(event){
                event.preventDefault();
                // Variable pour trouver l'élément "article"
                let getArticle = event.target.closest("article");
                ...
                let getModifyProduct = cart.find(element => element.id == dataId && element.color == dataColor);
                if (getModifyProduct) {
            ...
            itemDelete.addEventListener("click",function (event){
                ...
                if (window.confirm("Voulez-vous supprimer ce produit")) {
                    ...
                    let getDelelteItem = cart.filter(element => element.id != dataId || element.color != dataColor);
...
// Email
let emailReg = /^[a-zA-Z0-9_-]+@[a-zA-Z0-9-]{2,}[.][a-zA-Z]{2,3}$/
// Nom, prénom et ville
...
orderSubmit.addEventListener("click", function(event){
    ...
    // Sinon envoyer le formulaire
        ...
        // Création d'un objet qui contient les informations du client ainsi que l'id des produits commandés
        let userInfo = {
            contact : {
